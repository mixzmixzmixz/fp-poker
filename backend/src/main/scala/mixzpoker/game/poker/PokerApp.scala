...
object PokerApp {
  def of[F[_]: ConcurrentEffect: Logging](broker: Broker[F]): F[PokerApp[F]] = for {
    ...
    private def createGame(event: CreateGameEvent): EitherT[F, GameError, Unit] = for {
      _ <- EitherT.right(info"Create Game!")
//      gm <- PokerGameManager.create(event.gameId, event.settings, event.players, broker)
